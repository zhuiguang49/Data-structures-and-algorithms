...
public:
    ...
    public:
        ...
        const_iterator &operator--()
        {
            current = current->prev;  // 将迭代器移动到前一个节点
            return *this;
        }

        // 后置自减运算符：移动到前一个节点，但返回当前节点的拷贝
        const_iterator operator--(int)
        {
            const_iterator old = *this;  // 保存当前节点
            --(*this);  // 使用前置自减操作
            return old;
        }
    ...
    public:
        ...
        iterator &operator--()
        {
            this->current = this->current->prev;  // 将迭代器移动到前一个节点
            return *this;
        }
